...
const themes = {
  light: { class: { parent: styles.light, icon: styles.sun } },
  dark: { class: { parent: styles.dark, icon: styles.moon } },
};

export default function ThemeSwitch(): ReactElement {
  const [theme, setTheme] = useState(Object.keys(themes)[0]);
  const themeParams = themes[theme];
  ...
  return (
    <>
      <button
        type="button"
        className={[styles.toggle, themeParams.class.parent].join(' ')}
        onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}
      >
        <div className={[styles.icon, themeParams.class.icon].join(' ')}>
          <svg
